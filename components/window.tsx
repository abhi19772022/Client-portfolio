...
export default function Window({ window, isActive, onClose, onFocus, isDarkMode }: WindowProps) {
  ...
  const [preZoomState, setPreZoomState] = useState({ position, size })
  ...
  const toggleMaximize = () => {
    if (isZoomed) {
      ...
      setPosition(preZoomState.position)
      setSize(preZoomState.size)
    } else {
      // Save current state before zooming
      setPreZoomState({ position, size })

      // Calculate a larger "zoomed" size (80% of screen with proper margins)
      const margin = 40
      const availableWidth = window.innerWidth - margin * 2
      const availableHeight = window.innerHeight - 26 - 70 - margin // menubar + dock + margins

      const zoomedWidth = Math.min(availableWidth, size.width * 1.5)
      const zoomedHeight = Math.min(availableHeight, size.height * 1.5)

      // Center the zoomed window
      const newX = (window.innerWidth - zoomedWidth) / 2
      const newY = ((window.innerHeight - 70 - 26) - zoomedHeight) / 2 + 26 // Account for menubar
