...
export default function Terminal({ isDarkMode = true }: TerminalProps) {
  ...
  const executeCommand = (cmd: string) => {
    ...
    setHistory((prev) => [...prev, `himashu@macbook-pro ~ $ ${cmd}`, ""])
    ...
    switch (mainCommand) {
      ...
      case "about":
        setHistory((prev) => [
          ...
          "└─────────────────────────────────────┘",
          "",
          "I'm a passionate video editor with expertise in",
          "creating cinematic, engaging, and high-quality",
          "video content. I specialize in commercial edits,",
          "reels, motion graphics, and wedding films.",
          "With a keen eye for storytelling and visual",
          "aesthetics, I transform raw footage into",
  ...
  return (
    <div ref={terminalRef} className={`h-full ${bgColor} ${textColor} p-4 font-mono text-sm overflow-auto`}>
      ...
      <div className="flex">
        <span className="mr-2">himashu@macbook-pro ~ $</span>
        <input
          ref={inputRef}
          type="text"
          value={input}
